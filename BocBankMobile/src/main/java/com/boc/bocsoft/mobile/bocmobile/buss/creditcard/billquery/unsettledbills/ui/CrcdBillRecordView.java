...
public class CrcdBillRecordView extends MvpContainer<CrcdBillNPresenter> implements CrcdBillNContract.RecordView,BaseView<CrcdBillNPresenter>,PullToRefreshLayout.OnLoadListener, PinnedSectionListView.ClickListener {
    ...
    public CrcdBillRecordView(Context context) {
        super(context);
    }

    public CrcdBillRecordView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public CrcdBillRecordView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void initView() {
        ...
        billInfoItemView.setNoResultText(getResources().getString(R.string.boc_crcd_bill_y_bill_record_no_result));
    ...
    protected void initData() {
        ...
        transactionView.setShadowVisible(true);
     ...
     *  开始加载数据
     */
    public void startLoadData(AccountBean accountBean){
        ...
        getPresenter().crcdQueryFutureBillTotalIncome(billQueryModle);
    ...
    private void loadDetailListData() {
        ...
        getPresenter().crcdQueryFutureBill(billQueryModle);
    }
    ...
    @Override
    public void crcdQueryFutureBill(CrcdUnsettledBillDetailModel crcdBillQueryModel) {
    ...
    @Override
    public void crcdQueryFutureBillTotalIncome(List<CrcdUnsettledBillsModel> crcdUnsettledBillsModels) {
        ...
        if (crcdUnsettledBillsModels!=null&&crcdUnsettledBillsModels.size()>1){
            ...
            billInfoItemView.setRightContentText("收入",
                    ...
                    PublicCodeUtils.getCurrency(getContext(),crcdUnsettledBillsModels.get(1).getCurrency())+" "+MoneyUtils.transMoneyFormat(crcdUnsettledBillsModels.get(1).getCreditSum(),crcdUnsettledBillsModels.get(1).getCurrency()));
    ...
    private List<ShowListBean> generateTransactionBean(CrcdUnsettledBillDetailModel crcdBillQueryModel) {
        ...
        for (int i = 0; i < transBeanList.size(); i++) {
            LocalDate localDate = LocalDate.parse(transBeanList.get(i).getBookDate().replace("/","-"));
            ...
            if (i > 0) {
                tempTime = LocalDate.parse(transBeanList.get(i-1).getBookDate().replace("/","-")).format(DateFormatters.monthFormatter1);
            }
            if (tempTime.equals(formatTime)) {// child
                ...
                //存入类型
               if ("CRED".equals(transBeanList.get(i).getDebitCreditFlag())){
...
//                item.setContentRightAbove(PublicCodeUtils.getCurrency(getContext(), transBeanList.get(i).getBookCurrency()));
                item.setContentRightBelow(PublicCodeUtils.getCurrency(getContext(), transBeanList.get(i).getBookCurrency())+MoneyUtils.transMoneyFormat(transBeanList.get(i).getBookAmount(),transBeanList.get(i).getBookCurrency()));
                ...
                for (int j = 0; j < 2; j++) {
                    ...
                    if (j == 0) {
                        ...
                        itemFirst.setContentRightBelow(PublicCodeUtils.getCurrency(getContext(), transBeanList.get(i).getBookCurrency())+MoneyUtils.transMoneyFormat(transBeanList.get(i).getBookAmount(),transBeanList.get(i).getBookCurrency()));
                    }
