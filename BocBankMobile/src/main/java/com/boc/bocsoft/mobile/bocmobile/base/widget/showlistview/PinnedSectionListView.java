 ...
 * 交易列表
 */
public class PinnedSectionListView extends PullableListView {
    ...
    private void initView() {
        setDividerHeight(0);
        setOnScrollListener(mOnScrollListener);// 设置滚动监听
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mPinnedSection != null) {
            ...
            canvas.save();// 锁画布
            // TODO: 2016/10/12 view的高度
            int clipHeight = view.getHeight() +
                    (mShadowDrawable == null ? 0 : Math.min(mShadowHeight, mSectionsDistanceY));
            canvas.clipRect(pLeft, pTop, pLeft + view.getWidth(), pTop + clipHeight);// 裁剪画布
            canvas.translate(pLeft, pTop + mTranslateY);// 修改画布的原点
            ...
            canvas.restore();// 当前画布返回到上一个save()状态之前
        }
    }
    ...
    public void saveAdapter(ShowListAdapter adapter) {
