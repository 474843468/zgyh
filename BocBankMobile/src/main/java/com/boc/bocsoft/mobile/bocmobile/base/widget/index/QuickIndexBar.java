 ...
 * java代码中，请调用
 * quickIndexBar.setOnTouchLetterListener设置监听，然后在监听方法中写自己的逻辑
 * xml中，参照boc_fragment_choose_bank1.xml
 * 如下：
 * <com.boc.bocsoft.mobile.bocmobile.base.widget.index.QuickIndexBar1
...
public class QuickIndexBar extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            indexTitleTxt = a.getString(R.styleable.QuickIndexBar_strArr);
            a.recycle();
        }
        ...
        if (!TextUtils.isEmpty(indexTitleTxt)) {
            List<String> indexList = Arrays.asList(indexArr);
            List<String> newList = new ArrayList<String>();
            newList.addAll(indexList);
            String[] split = new String(indexTitleTxt).split(",");
            for (int i = split.length - 1; i >= 0; i--) {
                newList.add(0, split[i]);
            }
            indexArr = newList.toArray(new String[newList.size()]);
        }
    }
