...
public class RegexItem {
	...
	public RegexResult check(Context mContext, String input, boolean isMust){
		...
		// 空校验
		if (StringUtils.isEmpty(input) && isMust) {
			if(isMust){
				// 空的提示
		...
		}else if(StringUtils.isEmpty(input) && !isMust){
			return regexResult;
		}
		// 检查长度
		...
		//全角
		if(!isCanFullWord){
			boolean isHas = StringUtils.isContainFullWidthChar(input);
			if(isHas){
				regexResult.isAvailable = false;
				//含有全角字符
				regexResult.errorTips = getResString(tips_regex_ids);
		...
		if(!StringUtils.isEmpty(regex_str) && !StringUtils.isEmpty(input)){
			//
