...
public class DecodeHandler extends Handler {
    ...
    public DecodeHandler(ICaptureHandlerHelper captureHandlerHelper,
    ...
    public void handleMessage(Message message) {
        ...
        switch (message.what) {
            case QrcodeConstants.DECODE:
                decode((byte[]) message.obj, message.arg1, message.arg2);
                break;
            case QrcodeConstants.QUIT:
                running = false;
    ...
    private void decode(byte[] data, int width, int height) {
        ...
        Handler handler = captureHandlerHelper.getCaptureHandler();
        if (rawResult != null) {
            // Don't log the barcode contents for security.
            if (handler != null) {
                Message message =
                        Message.obtain(handler, QrcodeConstants.DECODE_SUCCEEDED, rawResult);
                Bundle bundle = new Bundle();
                bundleThumbnailLianlong(source, bundle);
                message.setData(bundle);
                message.sendToTarget();
            }
        } else {
            if (handler != null) {
                Message message = Message.obtain(handler, QrcodeConstants.DECODE_FAILED);
                message.sendToTarget();
            }
        }
    }

    private void bundleThumbnailLianlong(PlanarYUVLuminanceSource source, Bundle bundle) {
    ...
    public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data, int width, int height) {
        Rect rect = captureHandlerHelper.getCropRect();
        if (rect == null) {
            return null;
        }
