...
public class PrmsSaleSuccessActivity extends PrmsBaseActivity {
	...
	/**  卖出金额， 卖出币种*/
	private String sellAmount, sellAmountUnit;
	...
	private void init() {
		...
		LinearLayout tradeStyleLayout = (LinearLayout) findViewById(R.id.prms_trade_success_tradestyle_layout);
		tradeStyleLayout.setVisibility(View.GONE);
	...
	private void initDate() {
		...
		case PrmsControl.PRMS_TRADEMETHOD_LIMIT:
			...
			limitPriceTv.setText(PrmsControl.parsePriceByCurrency(
					limitPriceTemp, tradecurrencyStr));
	...
	private void setTradeAmount(Bundle extra){
		sellAmount = extra.getString(Prms.PRMS_QUSH_SUBMIT_BUYAMOUNT);
		sellAmountUnit = extra.getString(Prms.PRMS_QUERY_DEAL_BUYCURRENCYCODE);
		...
		trade_amount_tv.setText(getString(R.string.finc_hsAmount_colon));
		trade_amount.setText(StringUtil.parseStringCodePattern(sellAmountUnit, sellAmount, 2));
		trade_amount_unit.setText(LocalData.Currency.get(sellAmountUnit));
	}
	...
	private void setEnTrustDate(String endDate, String endHour) {
		entrustDateTv.setText(endDate + " " + endHour
				+ getString(R.string.forex_rate_currency_hour));
	}

	@Override
	public void onBackPressed() {
		startActivity(new Intent(PrmsSaleSuccessActivity.this,
...
//			finish();
			if(PrmsNewBaseActivity.prmsFlagGoWay == 2){
				...
				Intent intent = new Intent(PrmsSaleSuccessActivity.this, PrmsNewPricesActivity.class);
				startActivity(intent);
			}else if(PrmsNewBaseActivity.prmsFlagGoWay == 1){
				PrmsNewBaseActivity.prmsFlagGoWay = -1;
				ActivityTaskManager.getInstance().removeAllSecondActivity();
				Intent intent = new Intent(PrmsSaleSuccessActivity.this, PrmsNewPricesDetailActivity.class);
