...
/**
 * @ClassName: MyPositDrawMoneyActivity
 * @Description: 定期存款 “支取”和“续存”页面
 * @author luqp
 * @date 2015年8月13日10:11:14
 */
public class MyPositDrawMoneyActivity extends DeptBaseActivity {

	private static final String TAG = "MyPositDrawMoneyActivity";
	...
	/** 续存标识 */
	private String continueSaveFlag;
	/** 支取方式 */
	...
	/** 转账金额 */
	private String amount0fNotice = null;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		accountType = (String) DeptDataCenter.getInstance().getAccountContent().get(Comm.ACCOUNT_TYPE);

		convertType = (String) DeptDataCenter.getInstance().getCurDetailContent().get(Dept.NEW_NOTIFY_SAVE_CONVERT_TYPE);
...
//	}
	...
	private void initBottomLayout() {
		...
		final Map<String, Object> accountDetailMap = DeptDataCenter.getInstance().getCurDetailContent();
		...
		case 160:// 定活两便
			...
			dept_despite_amount_tv.setText(StringUtil.parseStringPattern(strMoney, 2));
			...
			EditTextUtils.setLengthMatcher(MyPositDrawMoneyActivity.this, messageEt, 50);
		...
		case 166:// 通知存款 分为约定转存和非约定转存
			...
			cdperiod = (String) accountDetailMap.get(Dept.CD_PERIOD);
			...
			// booksheetMoneyTv.setText(StringUtil.parseStringPattern(strBookBalance,
			// 2));
			// TextView 可用金额
			TextView checkoutMoney2Tv = (TextView) layoutLayout2.findViewById(R.id.dept_availablebalance_tv);
			availableBalance = (String) accountDetailMap.get(Dept.AVAILABLE_BALANCE);
			...
			availableBalance = StringUtil.parseStringCodePattern(strCurrency, availableBalance, 2);
			booksheetMoneyTv.setText(strBookBalance);
			checkoutMoney2Tv.setText(availableBalance);
			...
			if (ConstantGloble.CONVERTTYPE_N.equals(strConvertType)) {
				...
				noticeNoSpinner.setOnItemSelectedListener(new OnItemSelectedListener() {
					...
					public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
						...
						if (strNoticeNo.equals(str)) {
							...
							LogGloble.d("MyPositDrawMoneyActivity", "支取方式： " + drawMode);
							...
							if (drawMode.equals(DRAW_MODE_N)) {
								availableBalance = (String) curDetail.get(Dept.DRAW_AMOUNT);
								checkoutMoneyLayout.setVisibility(View.VISIBLE);
							...
							// 如果是韩元和日元 不显示小数点位
							availableBalance = StringUtil.parseStringCodePattern(strCurrency, availableBalance, 2);
							// Modify luqp 2015年12月4日17:07:18
							// drawMoney = StringUtil.parseStringCodePattern(strCurrency,drawMoney, 2);
							checkoutMoneyTv.setText(availableBalance);
							checkoutMoneyBottomTv.setText(availableBalance);
						}
					}
					...
					public void onNothingSelected(AdapterView<?> parent) {
						...
						drawMode = (String) DeptDataCenter.getInstance().getQueryNotifyCallBackList().get(0)
								.get(Dept.DRAW_MODE);
						LogGloble.d("MyPositDrawMoneyActivity", "默认支取方式： " + drawMode);
						checkoutWayTv.setText(LocalData.drawMode.get(drawMode));
					}
				});
			} else {// 约定转存
		...
		public void onClick(View v) {
			...
			if (!StringUtil.isNullOrEmpty(continueSaveFlag) && continueSaveFlag.equals("Y")) {
				...
				if (accountType.equals(ConstantGloble.ACC_TYPE_EDU)) {// 教育储蓄
					...
					// setLayoutHeight(mAccInListLayout, DeptDataCenter
					// .getInstance().getEducationAccountList());
					...
					// setLayoutHeight(mAccInListLayout, DeptDataCenter
					// .getInstance().getZerosaveAccountList());
				}
	...
	private OnItemClickListener clickAccEducationListItem = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			DeptDataCenter.getInstance().setAccInInfoNewMap(
					DeptDataCenter.getInstance().getEducationAccountList().get(position));
			BaseDroidApp.getInstanse().dismissMessageDialog();
			// 转入账户的省联号
			inAccountIbkNum = (String) DeptDataCenter.getInstance().getTranoutAccountList().get(position)
					.get(ConstantGloble.accountIbkNum);
			// 转入账户ID
			toAccountId = (String) DeptDataCenter.getInstance().getTranoutAccountList().get(position).get(Comm.ACCOUNT_ID);
			// //////////////////////////////////////////////////////////////////////////////
			// add by luqp 2015年11月23日10:29:38 手续费试算 非同省提示.
			// 用户选择转入账户与转出账户是否为同省, 同省不做处理,非同省弹出提示 如果用户同意确认与成功页面显示优惠后费用
			if (!StringUtil.isNullOrEmpty(outAccountIbkNum) && !StringUtil.isNullOrEmpty(inAccountIbkNum)
				...
				BaseDroidApp.getInstanse().dismissMessageDialog();
			} else { // TODO 提示用户非同省将会收取手续费 如果用户同意显示转出账户信息,如果不同意停留在账户选择列表
				BaseDroidApp.getInstanse().showErrorDialog(
						getResources().getString(R.string.saceregular_cost_Poundage_Prompt), R.string.saceregular_cancel,
						R.string.saceregular_countersign, OnClickListenerButton);
			}
			// /////////////////////////////////////////////////////////////////////////////
		}
	};
	...
	private OnItemClickListener clickWholesaveAccOutListItem = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> arg0, View view, int position, long id) {
			...
			// 转出账户的省联号 TODO
			outAccountIbkNum = (String) DeptDataCenter.getInstance().getWholesaveAccountList().get(position)
					.get(ConstantGloble.accountIbkNum);
			// ////////////////////////////////////////////////////////////////////
			// add by luqp 2015年11月23日10:27:17
			// 用户选择转入账户与转出账户是否为同省, 同省不做处理,非同省弹出提示 如果用户同意确认与成功页面显示优惠后费用
			if (!StringUtil.isNullOrEmpty(outAccountIbkNum) && !StringUtil.isNullOrEmpty(inAccountIbkNum)
					&& outAccountIbkNum.equals(inAccountIbkNum)) {
				// 同省不需要手续费,显示转入账户详情
				requestQueryAccountDetail(accountId);
				BaseDroidApp.getInstanse().dismissMessageDialog();
			} else { // 提示用户非同省将会收取手续费 如果用户同意显示转出账户信息,如果不同意停留在账户选择列表
				BaseDroidApp.getInstanse().showErrorDialog(
						getResources().getString(R.string.saceregular_cost_Poundage_Prompt), R.string.saceregular_cancel,
						R.string.saceregular_countersign, new OnClickListener() {
	...
	private OnClickListener nowExcuteBtnListener = new OnClickListener() {
		String availableAmount = (String) DeptDataCenter.getInstance().getCurDetailContent().get(Dept.AVAILABLE_BALANCE);
		...
		public void onClick(View v) {
			...
			// 原存单金额
			availableAmount = (String) DeptDataCenter.getInstance().getCurDetailContent().get(Dept.AVAILABLE_BALANCE);
			// 存折号
			passBookNumber = (String) DeptDataCenter.getInstance().getCurDetailContent().get(Dept.VOLUME_NUMBER);
			// 存单号
			cdNumber = (String) DeptDataCenter.getInstance().getCurDetailContent().get(Dept.CD_NUMBER);
			// 钞汇
			cashRemit = (String) DeptDataCenter.getInstance().getCurDetailContent().get(Dept.CASHREMIT);
			// 执行日期
			executeDate = (String) DeptDataCenter.getInstance().getCurDetailContent().get(Dept.INTEREST_ENDDATE);
			// 执行类型 立即执行
						...
						R.string.cancel_draw_money, R.string.continue_draw_money, new OnClickListener() {
							...
							public void onClick(View v) {
								if (v.getId() == R.id.retry_btn) {
									BaseDroidApp.getInstanse().dismissErrorDialog();
									BaseDroidApp.getInstanse().dismissMessageDialog();
									// skipSaveCheckOutConfirmActivity(drawMoney,
									// availableAmount);
									drawLumpReQuset();
								} else {
	...
	private void skipSaveCheckOutConfirmActivity(String drawMoney, String availableAmount) {
		...
		drawDate = (String) DeptDataCenter.getInstance().getCurDetailContent().get(Dept.INTEREST_ENDDATE);
	...
	private OnClickListener preDateNextBtnListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (accInInfoMap == null) {
				// Toast.makeText(context, chooseInNoMessage,
			...
			if (drawMode.equals(DRAW_MODE_N)) { // 部分支取
				...
				if (ConstantGloble.PRMS_CODE_RMB.equals(strCurrency)) {
					String errorMsg = MyPositDrawMoneyActivity.this.getResources().getString(R.string.notice_save_money);
					// //////////////////////////////////////////////////////////////////////
			...
			// !strNoticeNo.equals("")) {// 通知编号不为空时
			// LogGloble.e(TAG, "通知编号" + strNoticeNo);
			// String moneyD = checkoutMoneyEt.getText().toString().trim();//
			// 支取金额
			// double moneyDF = Double.valueOf(moneyD);// 转换成double类型，好进行比较
			// if (strBookBalance.contains(",")) {
			// strBookBalance = strBookBalance.replaceAll(",", "");
			// }
			// double sheetMoneyF = Double.parseDouble(strBookBalance);//
			// 把存单金额转换成double类型
			// if (moneyDF > sheetMoneyF) {// 当支取金额大于存单金额弹出提示
			// BaseDroidApp.getInstanse().showInfoMessageDialog(getResources().getString(R.string.dept_check_money));
			// return;
			// }
			// }
			...
			if (drawMode.equals(DRAW_MODE_N) && strNoticeNo != null) {
				BaseDroidApp.getInstanse().showErrorDialog(
						MyPositDrawMoneyActivity.this.getResources().getString(R.string.checkout_part_notify_message),
				...
				if (noticeFlag) {// 未到期提醒
					BaseDroidApp.getInstanse().showErrorDialog(
							MyPositDrawMoneyActivity.this.getResources().getString(R.string.checkout_notify_message_no_due),
	...
	private void showAccInListItemData() {
		...
		accInInfoMap = DeptDataCenter.getInstance().getAccInInfoNewMap();
	...
	private void initBottomBtnlayout(View view) {
		...
		nowExeBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				strMessage = messageEt.getText().toString().trim();
				String saveMoney = continueSaveTv.getText().toString().trim();

				if (TextUtils.isEmpty(saveMoney)) {// 校验续存金额是否为空
					BaseDroidApp.getInstanse().showInfoMessageDialog(
							MyPositDrawMoneyActivity.this.getResources().getString(R.string.add_amount_no_empty));
					return;
				}
				// 校验存款金额
	...
	public void communicationCallBack(int flag) {
		...
		case QUERY_ALL_IN_ACCOUNT_CALLBACK:// 查询转入账户返回 刷新界面
			...
			break;
		case QUERY_ACCOUNT_DETAIL_CALLBACK:
			if (accOutInfoMap != null) {
				DeptDataCenter.getInstance().setAccOutInfoMap(accOutInfoMap);
			}
			// 添加一个新的布局文件--转出账户详细信息
	...
	private void showAccWholeSaveOutListItemData() {
		...
		// 转出账户ID
		fromAccountId = (String) DeptDataCenter.getInstance().getAccountContent().get(Comm.ACCOUNT_ID);
		// 转出账户的省联号 TODO
		outAccountIbkNum = (String) DeptDataCenter.getInstance().getAccountContent().get(ConstantGloble.accountIbkNum);
		...
		// 存折号
		String strVolumeNumber = (String) DeptDataCenter.getInstance().getCurDetailContent().get(Dept.VOLUME_NUMBER);
		volumeNumberTv.setText(strVolumeNumber);
		// 存单号
		String strCdNumber = (String) DeptDataCenter.getInstance().getCurDetailContent().get(Dept.CD_NUMBER);
		cdNumberTv.setText(strCdNumber);

		// 可用余额
		availableBalance = (String) DeptDataCenter.getInstance().getCurDetailContent().get(Dept.AVAILABLE_BALANCE);
		...
		public void onClick(View v) {
			...
			case CustomDialog.TAG_SURE:
				if (drawMode.equals(DRAW_MODE_N) && strNoticeNo != null && !strNoticeNo.equals("")) {
					...
					if (noticeFlag) {// 未到期提醒
						BaseDroidApp.getInstanse().showErrorDialog(
								MyPositDrawMoneyActivity.this.getResources().getString(
	...
	private void skipSaveCheckMoneConfirmActivity() {
		String availableAmount = (String) DeptDataCenter.getInstance().getCurDetailContent().get(Dept.AVAILABLE_BALANCE);
		String drawMoney = availableAmount;
		drawMoney = (String) DeptDataCenter.getInstance().getCurDetailContent().get(Dept.AVAILABLE_BALANCE);
