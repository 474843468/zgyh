 ...
 * 预约日期查询 详情 与 执行日期查询 详情
 * @author lqp 2015年10月27日17:51:15
 * 修改页面跳转错误问题  修改ManageTransBaseActivity.java 为  DeptBaseActivity.java

 */
public class QueryDateDetailActivity1 extends DeptBaseActivity {
	/** 撤销 */
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		View view = mInflater.inflate(R.layout.tran_manage_exedate_detail_activity, null);
	...
	private void aquireData() {
		...
		QueryType = this.getIntent().getIntExtra(ConstantGloble.TRAN_MANAGER_QUERY_TYPE, -1);

		queryDetailQueryMap = TranDataCenter.getInstance().getQueryDetailCallBackMap();
		transactionId = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_TRANSACTIONID_RES);
		// periodicalSeq = (String) queryDetailQueryMap
		// .get(Tran.MANAGE_PREDETAIL_PERIODICALSEQ_RES);
		seqBat = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_BATSEQ_RES);
		transMode = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_TRANSMODE_RES);
		allAmount = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_ALLAMOUNT_RES);
		executeAmount = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_EXECUTEAMOUNT_RES);
		notExecuteAmount = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_NOTEXECUTEAMOUNT_RES);
		payerAccountNumber = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_PAYERACCOUNTNUMBER_RES);
		payerIbknum = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_PAYERIBKNUM_RES);
		payeeAccountNumber = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_PAYEEACCOUNTNUMBER_RES);
		currency = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_CURRENCY_RES);
		amount = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_AMOUNT_RES);
		channel = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_CHANNEL_RES);
		furInfo = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_FURINFO_RES);
		status = (String) queryDetailQueryMap.get(Tran.MANAGE_RECORDS_STATUS_RES);

		// 如果是 预约日期的 预约周期执行 paymentDate是从上个页面列表里的字段带过来的
		if (QueryType == PRE_DATE && transMode.equals(ConstantGloble.PREPERIODEXE)) {// 如果是预约周期
			paymentDate = this.getIntent().getStringExtra(ConstantGloble.PAYMENTDATE);

		} else {
			paymentDate = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_PAYMENTDATE_RES);
		}
		firstSubmitDate = (String) queryDetailQueryMap.get(Tran.MANAGE_PREDETAIL_FIRSTSUBMITDATE_RES);
	}
	...
	OnClickListener listener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			intent.setClass(QueryDateDetailActivity1.this, CancleDateConfirmActivity1.class);
	...
	private void displayTextView() {
		...
		needExeTv.setText(allAmount == null || "".equals(allAmount) ? "-" : allAmount);
		executedTv.setText(executeAmount == null || "".equals(executeAmount) ? "-" : executeAmount);
		notExeTv.setText(notExecuteAmount == null || "".equals(notExecuteAmount) ? "-" : notExecuteAmount);
		...
		// 如果币种是韩元 不显示小数点
		if (currency.equals(ConstantGloble.PRMS_CODE_HANYUAN1) || currency.equals(ConstantGloble.PRMS_CODE_HANYUAN2)) {
			transAmountTv.setText(StringUtil.parseStringPattern(amount, 0));
		} else {
