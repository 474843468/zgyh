...
public class ForexQuashQueryDateilActivity extends ForexBaseActivity {
	private static final String TAG = "ForexQuashQueryDateilActivity";
	/** 主视图布局 */
	 ...
	 */
	private void init() {
		rateInfoView = LayoutInflater.from(ForexQuashQueryDateilActivity.this).inflate(R.layout.forex_quash_query_detail, null);
		...
		cancelButton = (Button) findViewById(R.id.forex_query_deal_detailes_ok);
		...
		cancelView = findViewById(R.id.ll_btn);
	}
	...
	private void initDate() {
		...
		if (exchangeTranType.equals(seven) || exchangeTranType.equals(eight)) {
			...
			zhiSunRateView.setVisibility(View.GONE);
			weiTuoStatusView.setVisibility(View.VISIBLE);
			loseRateView.setVisibility(View.GONE);
		...
		if (exchangeTranType.equals(three) || exchangeTranType.equals(four)) {
			...
			if (ConstantGloble.FOREX_STATUSN.equals(firstStatus)) {
			...
			if (!StringUtil.isNull(firstStatus) && firstStatus.equals(ConstantGloble.FOREX_STATUSO)) {
				...
				if (ConstantGloble.FOREX_STATUSN.equals(secondStatus)) {
					cancelView.setVisibility(View.VISIBLE);
				}
			} else {
				if (!StringUtil.isNull(firstStatus) && LocalData.isForexQuashStateMap.containsKey(firstStatus)) {
	...
	private void initOnClick() {
		...
		cancelButton.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(ForexQuashQueryDateilActivity.this, ForexQuashConfirmActivity.class);
