 ...
 * 
 */
public class Price_DetailActivity extends SBRemitBaseActivity implements
		LoginWatcher {
	...
	/** 银行现汇卖出价 */
	private TextView sell_rate;
	/** 银行现汇买入价： */
	private TextView buy_rate;
	/** 银行现钞买入价： */
	private TextView buy_note_rate;
	/** 银行现钞卖出价： */
	private TextView sell_note_rate;
	/** 货币名称 */
	private String currency_nameString;
	/** 银行现汇卖出价 */
	private String sell_rateString;
	/** 银行现汇买入价： */
	private String buy_rateString;
	/** 银行现钞买入价： */
	private String buy_note_rateString;
	/** 银行现钞卖出价： */
	private String sell_note_rateString;
	...
	private void init() {
		...
		view = LayoutInflater.from(this).inflate(
				R.layout.sbremit_quotations_activity_detail, null);
		...
		sell_rate = (TextView) findViewById(R.id.sell_rate);
		buy_rate = (TextView) findViewById(R.id.buy_rate);
		buy_note_rate = (TextView) findViewById(R.id.buy_note_rate);
		sell_note_rate = (TextView) findViewById(R.id.sell_note_rate);

		manual_refresh.setVisibility(View.GONE);
		Intent intent = getIntent();
		currency_nameString = intent.getStringExtra(SBRemit.CURRENCY);
		sell_rateString = intent.getStringExtra(SBRemit.SELL_RATE);
		buy_rateString = intent.getStringExtra(SBRemit.BUY_RATE);
		buy_note_rateString = intent.getStringExtra(SBRemit.BUY_NOTE_RATE);
		sell_note_rateString = intent.getStringExtra(SBRemit.SELL_NOTE_RATE);
	...
	private void initListDatas() {
		...
		if (isLogin) {
			...
			setBtnRightClickListener(new OnClickListener() {
				...
				@Override
				public void onClick(View v) {
					startChooseAccountActivity(Price_DetailActivity.this);
				}
			});
	...
	public void requestSBRemitQuotationsCallback(Object resultObj) {
		...
		for (int i = 0; i < resultList.size(); i++) {
			...
			if (string != null) {
				if (string.equals("美元")) {
					updateTime = StringUtil.valueOf1((String) resultList.get(i)
							.get(SBRemit.UPDATETIME));
				}
			}
		}
	...
	public void refreshRateInfo() {
		...
		List<Map<String, Object>> resultList = SBRemitDataCenter.getInstance()
				.getExchangeOutlay();
		currency_name.setText(LocalData.moneyTypeValueKey
				.get(currency_nameString));
		if (sell_rateString .equals("0") || sell_rateString == null||sell_rateString .equals("")) {
			sell_rate.setText("-");
		} else {
			sell_rate.setText(sell_rateString);
		}

		if (buy_rateString .equals("0")  || buy_rateString == null||buy_rateString .equals("")) {
			buy_rate.setText("-");
		} else {
			buy_rate.setText(buy_rateString);
		}

		if (buy_note_rateString .equals("0") || buy_note_rateString == null||buy_note_rateString .equals("")) {
			buy_note_rate.setText("-");
		} else {
			buy_note_rate.setText(buy_note_rateString);
		}

		if (sell_note_rateString .equals("0") || sell_note_rateString == null||sell_note_rateString .equals("")) {
			sell_note_rate.setText("-");
		} else {
			sell_note_rate.setText(sell_note_rateString);
		}
...
//				intent.setClass(Price_DetailActivity.this, LoginActivity.class);
//				startActivityForResult(intent, REQUEST_LOGIN_CODE);
				BaseActivity.getLoginUtils(Price_DetailActivity.this).exe(new LoginTask.LoginCallback() {
