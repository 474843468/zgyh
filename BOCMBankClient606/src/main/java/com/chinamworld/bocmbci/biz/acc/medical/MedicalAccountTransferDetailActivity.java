 ...
 * 
 */
public class MedicalAccountTransferDetailActivity extends AccBaseActivity
...
//	}
	...
	private void init() {
		...
		acc_frame_left = (ImageView) rl_query_transfer
				.findViewById(R.id.acc_frame_left);
		acc_btn_goitem = (ImageView) rl_query_transfer
				.findViewById(R.id.acc_btn_goitem);
		gallery = (CustomGallery) rl_query_transfer
				.findViewById(R.id.viewPager);
		// 条件视图
		query_condition = (LinearLayout) rl_query_transfer
				.findViewById(R.id.ll_query_condition);
		query_condition.setBackgroundResource(R.drawable.img_bg_query_j);
		// 查询条件页面初始化
		currency_choose = (Spinner) rl_query_transfer
				.findViewById(R.id.acc_currency_spinner);
		cashRemit_choose = (Spinner) rl_query_transfer
		...
		LinearLayout ll_up = (LinearLayout) rl_query_transfer
				...
				if (!StringUtil.isNullOrEmpty(queryCodeList)
					...
					if (mycurrency.equals(rmb)) {
						cashRemitadapter = new ArrayAdapter(
								MedicalAccountTransferDetailActivity.this,
		...
		public void onClick(View v) {
			...
			case R.id.tv_text2:
				...
				if (listIn == null || listIn.size() == 0) {
					BaseDroidApp.getInstanse().showInfoMessageDialog(
							MedicalAccountTransferDetailActivity.this.getString
				...
				AccountTransferAdapter indetailadapter = new AccountTransferAdapter(
						MedicalAccountTransferDetailActivity.this, listIn,
			...
			case R.id.tv_text3:
				...
				AccountTransferAdapter outdetailadapter = new AccountTransferAdapter(
						MedicalAccountTransferDetailActivity.this, listOut,
						queryMap.get(Acc.QUERYTRANSFER_ACC_CURRENCY_REQ));
	...
	public void transferListCallBack(Object resultObj) {
		...
		if (transferList == null || transferList.size() == 0) {
			...
			detailadapter = new AccountTransferAdapter(
					MedicalAccountTransferDetailActivity.this, transferList,
					queryMap.get(Acc.QUERYTRANSFER_ACC_CURRENCY_REQ));
			lv_acc_query_result.setAdapter(detailadapter);
