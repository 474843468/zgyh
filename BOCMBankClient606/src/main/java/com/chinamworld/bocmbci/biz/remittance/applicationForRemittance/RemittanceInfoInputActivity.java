...
import com.chinamworld.bocmbci.biz.remittance.fragment.PayeeInfoFragment;
import com.chinamworld.bocmbci.biz.remittance.fragment.RemitterInfoFragment;
 ...
 * @author Zhi
 */
public class RemittanceInfoInputActivity extends RemittanceBaseActivity implements NeedAccDetailListener {

	// TODO -------------------成员变量------------------------------//

	private static final String TAG = "RemittanceInfoInputActivity";
	/** 汇款人信息输入页面片段 */
	private RemitterInfoFragment remitterFrag;
	/** 收款人信息输入页面片段 */
	private PayeeInfoFragment payeeFrag;
	/** 交易信息输入页面片段 */
	private TransationInfoFragment transationFrag;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setLeftSelectedPosition("crossBorderRemit_1");
		setTitle(this.getString(R.string.remittance_apply));
		...
		findViewById(R.id.ib_back).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				//ModelBoc.gotoMainActivity(RemittanceInfoInputActivity.this);
				//goToMainActivity();
				finish();
			}
		});
	...
	private void initView() {
		...
		if (remitterFrag == null) {
			remitterFrag = new RemitterInfoFragment();
		}
		if (payeeFrag == null) {
			payeeFrag = new PayeeInfoFragment();
		}
		if (transationFrag == null) {
			transationFrag = new TransationInfoFragment();
			transationFrag.setNeedAccDetailListener(this);
			remitterFrag.setAccChangeView(new AccChangeListener[] { transationFrag });
			transationFrag.setAccSelectionListener(remitterFrag);
		}
		boolean state = getIntent().getBooleanExtra(RemittanceContent.JUMPFLAG, false);
		if (state) {
			remitterFrag.setState(1, false, false);
			payeeFrag.setState(1, false);
			transationFrag.setState(1, false);
		} else {
			remitterFrag.setState(0, false, true);
			payeeFrag.setState(0, false);
			transationFrag.setState(0, false);
		}
		ft.replace(R.id.ll_input1, remitterFrag);
		ft.replace(R.id.ll_input2, payeeFrag);
		ft.replace(R.id.ll_input3, transationFrag);
		ft.commit();

		payeeFrag.setOnAreaSelectListener(new OnAreaSelectListener() {
			@Override
			public void onAreaSelect(int position) {
				transationFrag.onAreaSelect(position);
			}
		});
		...
		//判断是否是模板跳转过来的 ，如果是，不显示转账汇款提示信息
		boolean isOhter = getIntent().getBooleanExtra("isOhter",false);
		if (isOhter) {
			needAccListView = remitterFrag;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode == RemittanceContent.CHOOSE_CHOUNTRY) {
			...
			remitterFrag.setState(1, true, false);
			payeeFrag.setState(1, true);
			transationFrag.setState(1, false);
		} else if (resultCode == RemittanceContent.QUERY_SWIFT) {
			payeeFrag.onActivityResult(requestCode, resultCode, data);
	...
	public boolean doHttpErrorHandler(String method, BiiError biiError) {
		...
		if ("CCSS.S0012".equals(errorCode)) {
			...
			BaseDroidApp.getInstanse().showInfoMessageDialog("您所持认证工具不支持此交易，如您需要使用此交易，请前往柜台申请中银e令或中银e盾");
		} else if ("AccQueryDetailAction.NoSubAccount".equals(errorCode)) {
			BaseDroidApp.getInstanse().showMessageDialog("您的账户无存单信息", new OnClickListener() {

				@Override
				public void onClick(View v) {
					BaseDroidApp.getInstanse().dismissErrorDialog();
					remitterFrag.spPayAcctSetZero();
					transationFrag.initViewZero();
				}
			});
		}
		return true;
	...
	private void ibanFormatCheck() {
		...
		setOnIbanFormatCheckListener(new OnIbanFormatCheckListener() {
			...
			public void onIbanFormatCheck(boolean isPass) {
				if (isPass) {
					Map<String, Object> params = new HashMap<String, Object>();
					params.put(Remittance.PAYEEACTNO, RemittanceDataCenter.getInstance().getSubmitParams().get(Remittance.PAYEEACTNO));
					params.put(Remittance.PAYEEBANKSWIFT, RemittanceDataCenter.getInstance().getSubmitParams().get(Remittance.PAYEEBANKSWIFT));
					getHttpTools().requestHttp(Remittance.PSNIBANBICMATCHCHECK, "requestPsnIbanBicMatchCheckCallBack", params, false);
				} else {
		...
		Map<String, Object> params = new HashMap<String, Object>();
		params.put(Remittance.PAYEEACTNO, RemittanceDataCenter.getInstance().getSubmitParams().get(Remittance.PAYEEACTNO));
		getHttpTools().requestHttp(Remittance.PSNIBANFORMATCHECK, "requestPsnIbanFormatCheckCallBack", params, false);
	}

	/** 判断收款人地区是否是AU或CA */
	private void checkAUOrCA() {
		// String accNumber = (String)
		// RemittanceDataCenter.getInstance().getSubmitParams().get(Remittance.PAYEEACTNO);
	...
	private OnClickListener nextListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (!remitterFrag.remittanceConfirm()) {
				return;
			}
			if (!payeeFrag.remittanceConfirm()) {
				return;
			}
			if (!transationFrag.remittanceConfirm()) {
				return;
			}
	...
	public void setChooseAreaListener(OnAreaSelectListener listener) {
		payeeFrag.setOnAreaSelectListener(listener);
	}
	...
	public void requestCommConversationIdCallBack(Object resultObj) {
		...
		getHttpTools().registErrorCode(Acc.ACC_GETSECURITYFACTOR_API, "safetyComBin.nosSafetyComins");
	}
	...
	private void requestSecurityFactor() {
		...
		map.put(Acc.ACC_GETSECURITY_SERVICEID_REQ, Remittance.ServiceId);
	...
	public void requestPsnCommonQueryAllChinaBankAccountCallBack(Object resultObj) {
		BaseHttpEngine.dissMissProgressDialog();
		getHttpTools();
		List<Map<String, Object>> accList = (List<Map<String, Object>>) HttpTools.getResponseResult(resultObj);
		List<Map<String, Object>> haveAccList = new ArrayList<Map<String, Object>>();
		if (StringUtil.isNullOrEmpty(accList)) {
			BaseDroidApp.getInstanse().showInfoMessageDialog("您当前没有可以办理跨境汇款服务的账户");
			return;
		}
		...
		if (haveAccList.size() > 0) {
			...
			BaseDroidApp.getInstanse().showInfoMessageDialog("因您名下的关联账户中无可用的活一本或主账户是活一本的借记卡，故无法使用该功能服务");
			return;
		}
		LogGloble.i(TAG, RemittanceDataCenter.getInstance().getAccList().toString());
	...
	public void requestPsnOtherNameOfCustQueryCallBack(Object resultObj) {
		...
		BaseHttpEngine.dissMissProgressDialog();
		remitterFrag.setRemitterName(name);
	}
	...
	public void requestPsnTransGetInternationalTransferCommissionChargeCallBack(Object resultObj) {
		...
		// if (resultMap.get(Remittance.GETCHARGEFLAG).equals("0")) {
		// BaseHttpEngine.dissMissProgressDialog();
		// BaseDroidApp.getInstanse().showInfoMessageDialog("费用试算失败");
		// return;
		// }
	...
	public void requestPsnPayeeBankBSBCheckCallBack(Object resultObj) {
		...
		if (resultStr.equals("01") || resultStr.equals("02") || resultStr.equals("03")) {
			BaseHttpEngine.dissMissProgressDialog();
			BaseDroidApp.getInstanse().showErrorDialog("为确保您的款项顺利汇出，请确认您填写的收款行行号是否正确。", R.string.remittance_btn_jixutijiao, R.string.remittance_btn_backCheck, new OnClickListener() {
	...
	public void requestPsnIbanFormatCheckCallBack(Object resultObj) {
		...
		if (onIbanFormatCheckListener != null) {
			...
			if (checkResult.equals("1") || checkResult.equals("2") || checkResult.equals("3")) {
	...
	public void requestPsnIbanBicMatchCheckCallBack(Object resultObj) {
		...
		if (resultStr.equals("3")) {
			...
			BaseDroidApp.getInstanse().showErrorDialog(getResources().getString(R.string.remittance_IbanBicMatchCheck), R.string.remittance_btn_jixutijiao, R.string.remittance_btn_backCheck, new OnClickListener() {
				@Override
				public void onClick(View v) {
					switch (Integer.parseInt(v.getTag() + "")) {
					case CustomDialog.TAG_SURE:
	...
	private boolean isContainUsb(String tokenId) {
		if (Acc.ECURITY_8.equals(tokenId) || Acc.ECURITY_32.equals(tokenId) || Acc.ECURITY_40.equals(tokenId) || Acc.ECURITY_4.equals(tokenId) || Acc.ECURITY_12.equals(tokenId) || Acc.ECURITY_36.equals(tokenId) || Acc.ECURITY_96.equals(tokenId))
			return false;
		return true;
	}
	...
	public void doSecurityFactorNull() {
		...
		BaseDroidApp.getInstanse().showMessageDialog("您所持认证工具不支持此交易，如您需要使用此交易，请前往柜台申请中银e令或中银e盾。", new OnClickListener() {
	...
	public void requestGetSecurityFactorCallBack(Object resultObj) {
		...
		if (biiResponseBody.getResult() == null || "".equals(biiResponseBody.getResult())) {
		...
		// 默认因子
		@SuppressWarnings("unchecked")
		Map<String, String> defaultCombin = (Map<String, String>) result.get(Acc.ACC_GETSECURITY_DEFAULTCOMBIN_RES);
		BaseDroidApp.getInstanse().setDefaultCombinId(String.valueOf(-1));
		if (defaultCombin != null && !isContainUsb(defaultCombin.get(Acc.ECURITYID))) {// 不包含USB组合
			BaseDroidApp.getInstanse().setDefaultCombinId(defaultCombin.get(Acc.ECURITYID));
		}
		@SuppressWarnings("unchecked")
		List<Map<String, String>> combinList = (List<Map<String, String>>) result.get(Acc.ACC_GETSECURITY_COMBINLIST_RES);
			...
			if (length == 1 && combinList.get(0).get(Acc.ECURITYID) == "96") {
				...
				for (int i = 0; i < length; i++) {
					if (!isContainUsb(combinList.get(i).get(Acc.ECURITYID))) {// 不是USB组合就添加
						if (!BaseDroidApp.getInstanse().getSecurityIdList().contains(combinList.get(i).get(Acc.ECURITYID))) {
		...
		getHttpTools().requestHttp(Comm.QRY_ALL_BANK_ACCOUNT, "requestPsnCommonQueryAllChinaBankAccountCallBack", params, false);
	}
	...
	public void queryRandomNumberCallBack(Object resultObj) {
		...
		Intent intent = new Intent(RemittanceInfoInputActivity.this,RemittanceConfirmActivity.class);
	...
	public void requestPsnQryInternationalTrans4CNYCountryCallBack(Object resultObj) {
		...
		startActivityForResult(new Intent(this, RemittanceChooseCountryActivity.class), RemittanceContent.CHOOSE_CHOUNTRY);
...
//			startActivity(new Intent(this, NationalTransferLimitDialogActivity.class).putExtra("CURRENCY", transationFrag.getCurrency()));
//			Log.v("BiiHttpEngine",transationFrag.getCurrency()+"---requestPsnQueryNationalTransferLimitCallBack");
//			overridePendingTransition(R.anim.no_animation, R.anim.no_animation);
//		} else if (limitFlag == 1) {
//			// 汇款金额
...
//				BaseHttpEngine.dissMissProgressDialog();
//				if (transationFrag.getCurrency().equals("014")) {
...
//	}
	...
	public void transferTemplateQueryCallBack(Object resultObj) {
		...
		startActivityForResult(new Intent(this, UseModelDialogActivity.class).putExtra("isOversearChinaBankModel",1), RemittanceContent.CHOOSE_MODE);
	}
}
