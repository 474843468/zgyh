...
import com.chinamworld.bocmbci.biz.tran.mobiletrans.adapter.MobileQueryTransferAdapter;
 ...
 * 
 */
public class MobileTransferQueryActivity extends TranBaseActivity implements
		OnClickListener {
	private View view;
	/** 查询开始日期 */
	...
	/** 查询适配器 */
	private MobileQueryTransferAdapter detailAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		view = addView(R.layout.tran_mobile_querytransfer);
	...
	public void init() {
		...
		acc_query_transfer_layout = (RelativeLayout) LayoutInflater.from(
				...
				R.layout.trans_mobile_record_condition, null);
		//查询结果头布局
		LinearLayout acc_query_result_condition = (LinearLayout) view
...
//		PopupWindowUtils.getInstance().showQueryPopupWindowFirst();
		// 条件视图
		query_condition = (LinearLayout) acc_query_transfer_layout
				.findViewById(R.id.ll_query_condition);
		query_condition.setBackgroundResource(R.drawable.img_bg_query_j);
		// 查询条件页面初始化
		tv_startdate = (TextView) acc_query_transfer_layout
				.findViewById(R.id.acc_query_transfer_startdate);
		tv_enddate = (TextView) acc_query_transfer_layout
				.findViewById(R.id.acc_query_transfer_enddate);
		btn_query_onweek = (Button) acc_query_transfer_layout
				.findViewById(R.id.btn_acc_onweek);
		btn_query_onmonth = (Button) acc_query_transfer_layout
				.findViewById(R.id.btn_acc_onmonth);
		btn_query_threemonth = (Button) acc_query_transfer_layout
				.findViewById(R.id.btn_acc_threemonth);
		btn_query = (Button) acc_query_transfer_layout
				.findViewById(R.id.btn_acc_query_transfer);
		img_up = (ImageView) acc_query_transfer_layout
				.findViewById(R.id.acc_query_up);
		query_status = (Spinner) acc_query_transfer_layout
				.findViewById(R.id.sp_trans_cremit_manage_trans_records);
		// 三天前的日期
		...
		// 收起事件监听
		LinearLayout ll_up = (LinearLayout) acc_query_transfer_layout
	...
	public void spinnerInit() {
		...
		ArrayAdapter<String> currencyAdapter = new ArrayAdapter<String>(this,
				...
				LocalData.mobileTransStatusUpList);
		currencyAdapter
		...
		query_status.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View view,
				...
				querystatus = LocalData.mobileTranStatusUpmap
						.get(LocalData.mobileTransStatusUpList.get(position));
			}

			@Override
			public void onNothingSelected(AdapterView<?> parent) {
				querystatus = LocalData.mobileTranStatusUpmap
						.get(LocalData.mobileTransStatusUpList.get(0));
			}
		});
	...
	public void initfirst() {
		...
		btn_load_more.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				requestMobileTransferQueryUnSubmitTransForMore();
			}
		});
		querystatus = LocalData.mobileTranStatusUpmap
				.get(LocalData.mobileTransStatusUpList.get(0));
		tv_query_status.setText(LocalData.mobileTransStatusUpList.get(0));
	 ...
	 * 手机号转账交易查询回调
	 * */
	public void requestMobileTransferQueryUnSubmitTrans() {

		BiiRequestBody biiRequestBody = new BiiRequestBody();
		biiRequestBody.setMethod(Tran.PSN_MOBILE_TRANSFER_QUERY_UNSUBMITTRANS);

		Map<String, String> map = new HashMap<String, String>();
		map.put(Tran.SERVICEID_REQ, ConstantGloble.PB035);
		map.put(Tran.BEIGIN_DATE, startdate);
		map.put(Tran.ENDDATE, enddate);
		map.put(Tran.STATUS, querystatus);
		map.put(Tran.PAYEE_CURRENTINDEX_REQ,
		...
		HttpManager.requestBii(biiRequestBody, this,
				"requestMobileTransferQueryUnSubmitTransCallBack");
	}
	...
	public void requestMobileTransferQueryUnSubmitTransCallBack(Object resultObj) {
...
//			PopupWindowUtils.getInstance().dissMissQueryPopupWindow();
			...
			tv_query_status.setText(LocalData.mobileTranStatusUpmap2
					.get(querystatus));
			tv_acc_query_result_date.setText(startdate + "-" + enddate);
			detailAdapter = new MobileQueryTransferAdapter(this, transferList);
	...
	OnItemClickListener onListClickListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int position,
				long arg3) {
			TranDataCenter.getInstance().setMobileTransDetailMap(
					transferList.get(position));
			String status = (String) transferList.get(position).get(
					Tran.MOBILE_STATUS_RES);
//			if (status.equals(ConstantGloble.STATUS_FAIL)) {
//				// 已回复 不可点击
//			} else {
				Intent intent = new Intent(MobileTransferQueryActivity.this,
						MobileTransferDetailActivity.class);
...
//			}
	 ...
	 * 手机号转账交易查询回调
	 * */
	public void requestMobileTransferQueryUnSubmitTransForMore() {
		BiiRequestBody biiRequestBody = new BiiRequestBody();
		biiRequestBody.setMethod(Tran.PSN_MOBILE_TRANSFER_QUERY_UNSUBMITTRANS);
		...
		map.put(Tran.BEIGIN_DATE, startdate);
		map.put(Tran.ENDDATE, enddate);
		map.put(Tran.STATUS, queryMap.get(Tran.STATUS));
		map.put(Tran.PAYEE_CURRENTINDEX_REQ, String.valueOf(loadNumber));
		map.put(Tran.PAYEE_PAGESIZE_REQ, ConstantGloble.LOAN_PAGESIZE_VALUE);
		biiRequestBody.setParams(map);
		BiiHttpEngine.showProgressDialog();
		HttpManager.requestBii(biiRequestBody, this,
				"requestMobileTransferQueryUnSubmitTransForMoreCallBack");
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void requestMobileTransferQueryUnSubmitTransForMoreCallBack(
			Object resultObj) {
		BiiResponse biiResponse = (BiiResponse) resultObj;
		List<BiiResponseBody> biiResponseBodys = biiResponse.getResponse();
		BiiResponseBody biiResponseBody = biiResponseBodys.get(0);
	...
	OnClickListener chooseDateClick = new OnClickListener() {
		...
		public void onClick(final View v) {
			...
			DatePickerDialog dialog = new DatePickerDialog(
					MobileTransferQueryActivity.this, new OnDateSetListener() {
...
//			PopupWindowUtils.getInstance().showQueryPopupWindow();
	...
	public void onClick(View v) {
		...
		case R.id.btn_acc_onweek:
			...
			// 请求账户明细信息
			requestMobileTransferQueryUnSubmitTrans();
			break;
	...
	public void queryDetail() {
		...
		if (QueryDateUtils.compareDateOneYear(startdate, dateTime)) {
			...
			BaseDroidApp.getInstanse().showInfoMessageDialog(
					MobileTransferQueryActivity.this
