...
public class ZextendAgreeDetailActivity extends BociBaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 添加布局
		view = addView(R.layout.boc_zidong_detail);
		// 界面初始化
		init();
	}
	...
	public void init() {
		...
		tv_minAmount.setText(StringUtil.parseStringCodePattern(
				(String) detailMap.get(BocInvt.BOC_EXTEND_PROCUR_RES),
				(String) detailMap.get(BocInvt.BOC_EXTEND_MINAMOUNT_RES), 2));
		tv_maxAmount.setText(StringUtil.parseStringCodePattern(
				(String) detailMap.get(BocInvt.BOC_EXTEND_PROCUR_RES),
				(String) detailMap.get(BocInvt.BOC_EXTEND_MAXAMOUNT_RES), 2));
	...
	public void moreClick(String text, View v) {
		...
		if (text.equals(bocSerlistList.get(2))) {
			// 开始
			Intent intent = new Intent(ZextendAgreeDetailActivity.this,
					ZextendAgreeConfirmActivity.class);
	...
	public void requestSystemDateTimeCallBack(Object resultObj) {
		...
		if (!StringUtil.isNull(dateTime)) {
			Intent intent = new Intent(ZextendAgreeDetailActivity.this,
					ZextendAgreeEditInputActivity.class);
