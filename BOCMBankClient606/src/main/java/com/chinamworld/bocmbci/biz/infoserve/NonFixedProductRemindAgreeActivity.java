 ...
 * 
 */
public class NonFixedProductRemindAgreeActivity extends InfoServeBaseActivity {
	private static final String TAG = "NonFixedProductRemindAgreeActivity";
	/**
	 * quickTrade：关闭
	 */
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setTitle(R.string.infoserve_non_fixed_product_remind_agree_title);

		addView(R.layout.service_non_fixed_product_remind_agree_info);
	...
	private void init() {
		...
		agreeButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// // 跳转到修改界面
				// Intent intent = new Intent(
				// NonFixedProductRemindAgreeActivity.this,
				// NonFixedProductRemindSetActivity.class);
				// intent.putExtra(Push.NON_FIXED_OPT_FLAG,
				// Push.NON_FIXED_OPT_ADD);
				// intent.putExtra(Comm.ACCOUNT_ID,
				// getIntent().getStringExtra(Comm.ACCOUNT_ID));
				// intent.putExtra(Push.NON_FIXED_CURRENCY,
				// "CNY");
				// startActivity(intent);
				// // 跳转到大额到账提醒设置画面
				setResult(101);
