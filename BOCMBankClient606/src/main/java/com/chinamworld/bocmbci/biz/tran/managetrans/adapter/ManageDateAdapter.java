...
public class ManageDateAdapter extends BaseAdapter {
	...
	public ManageDateAdapter(Context context,
			List<Map<String, Object>> queryResultList) {
		this.mContext = context;
		this.queryResultList = queryResultList;

		// LogGloble.i(ManageDateAdapter.class.getSimpleName(),
		// "ManageDateAdapter...");
	}
	...
	public Object getItem(int position) {
		...
		/*
		 * if(position == 0) { return "headTitle"; } else { return
		 * queryResultList.get(position-1); }
		 */
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			// holder.content =
			// (LinearLayout)convertView.findViewById(R.id.ll_pre_date_listview_content);
			// holder.header =
			// (LinearLayout)convertView.findViewById(R.id.pre_date_listview_header);
			...
			PopupWindowUtils.getInstance().setOnShowAllTextListener(mContext,
					holder.dateView);
			holder.batSeqView = (TextView) convertView
					.findViewById(R.id.tv_transeq_manage_predate_list_item);
			PopupWindowUtils.getInstance().setOnShowAllTextListener(mContext,
					holder.batSeqView);
			holder.payeeAccountNameView = (TextView) convertView
					.findViewById(R.id.tv_prepayee_manage_predate_list_item);
			PopupWindowUtils.getInstance().setOnShowAllTextListener(mContext,
					holder.payeeAccountNameView);
		...
		if (holder != null) {
			...
			if (map != null) {
				holder.dateView.setText((String) map
						.get(Tran.MANAGE_PRE_FIRSTSUBMITDATE_RES));// //DateFormat.format(inFormat,
																	// inDate)
				holder.batSeqView.setText((String) map
						.get(Tran.MANAGE_PRE_BATSEQ_RES));
				holder.payeeAccountNameView.setText((String) map
