 ...
 * 
 */
public class ManageTransRecordActivity1 extends ManageTransBaseActivity
	...
	private View ll_result_list= null;
	/** 查询时间 */
	...
	private String reexchangestate;//	退汇交易状态 0表示未退汇1表示已经退汇 
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		View view = mInflater.inflate(
				R.layout.tran_manage_trans_records_activity, null);
...
//	}
	...
	private void setViewOnClickListener() {
		...
		ArrayAdapter<String> adapter1 = new ArrayAdapter<String>(this,
				R.layout.custom_spinner_item, LocalData.transTypeList);
...
//					searchLayout.startAnimation(animation_up);
	...
	private OnClickListener resultOnClickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			// LinearLayout
			// ll_query_bg=(LinearLayout)findViewById(R.id.ll_query_bg);
			// ll_query_bg.setBackgroundResource(R.drawable.img_bg_query_no);
			// 显示查询条件
...
//			searchLayout.startAnimation(animation_down);
	...
	private void initanimation() {
		animation_up = new AnimationUtils().loadAnimation(
				ManageTransRecordActivity1.this, R.anim.query_exit);
		animation_down = new AnimationUtils().loadAnimation(
				ManageTransRecordActivity1.this, R.anim.query_enter);
	}
	...
	private void setupView() {
		...
		ll_result_list= findViewById(R.id.ll_result_list);
	...
	public void onClick(View v) {
		...
		case R.id.tv_startdate_trans_records:
			...
			DatePickerDialog startDateDialog = new DatePickerDialog(
					ManageTransRecordActivity1.this, onStartDateSetListener,
		...
		case R.id.tv_enddate_trans_records:
			...
			DatePickerDialog endDateDialog = new DatePickerDialog(
					ManageTransRecordActivity1.this, onEndDateSetListener,
		...
		case R.id.btn_query_trans_records:
			resetData();
			startDateStr = startDateTv.getText().toString().trim();
			endDateStr = endDateTv.getText().toString().trim();
			// 判断时间
			if (!QueryDateUtils.compareDateOneYear(startDateStr, dateTime)) {
				// 开始日期在系统日期前一年以内
				BaseDroidApp.getInstanse().showInfoMessageDialog(
						ManageTransRecordActivity1.this
								.getString(R.string.acc_check_start_enddate));
				return;
			} else if (!QueryDateUtils.compareDate(endDateStr, dateTime)) {
				// 结束日期在系统日期之前
				BaseDroidApp.getInstanse().showInfoMessageDialog(
						ManageTransRecordActivity1.this
								.getString(R.string.acc_check_enddate));
				return;
			} else if (!QueryDateUtils.compareDate(startDateStr, endDateStr)) {
				// 开始日期在结束日期之前
				BaseDroidApp.getInstanse().showInfoMessageDialog(
						ManageTransRecordActivity1.this
								.getString(R.string.acc_query_errordate));
				return;
			} else if (!QueryDateUtils.compareDateThree(startDateStr,
					endDateStr)) {
				// 起始日期与结束日期最大间隔为三个自然月
				BaseDroidApp.getInstanse().showInfoMessageDialog(
						ManageTransRecordActivity1.this
	 ...
	 * 转账记录查询 req
	 */
	private void RequestForTransQueryTransferRecord(String startDateStr,
			String endDateStr) {
		BiiRequestBody biiRequestBody = new BiiRequestBody();
		biiRequestBody.setMethod(Tran.TRANSQUERYTRANSFERRECORD);
		Map<String, String> map = new HashMap<String, String>();
		if (curIndex > 0) {
			transTypeName = serachRangeTV.getText().toString();
			transTypeCode = LocalData.transType.get(transTypeName);
		}
		...
		HttpManager.requestBii(biiRequestBody, this,
				"transQueryTransferRecordCallBack");
	}
	...
	public void transQueryTransferRecordCallBack(Object resultObj) {
		...
		if (curIndex + pageSize < preRecordCount) {
			if (queryResultLv.getFooterViewsCount() > 0) {
	...
	private void showQueryResult(List<Map<String, Object>> list) {
		...
		preDateAdapter = new ManageRecordsAdapter1(
				ManageTransRecordActivity1.this, list);
	...
	public void requestForTransQueryTransferRecordDetail(int position) {
		...
		Map<String, Object> queryTransferRecord = queryTransferRecordList
				.get(position);
		transId = (String) queryTransferRecord
				.get(Tran.MANAGE_RECORDS_TRANSACTIONID_RES);
		reexchangestate=(String) queryTransferRecord.get(Tran.MANAGE_REMITRETURN_REEXCHANGESTATE_RES);
		Map<String, String> map = new HashMap<String, String>();
		map.put(Tran.MANAGE_RECORDSDETAIL_transId_REQ, transId);
		biiRequestBody.setParams(map);
		BaseHttpEngine.showProgressDialog();
		HttpManager.requestBii(biiRequestBody, this,
				"transQueryTransferRecordDetailCallBack");
	}
	...
	public void transQueryTransferRecordDetailCallBack(Object resultObj) {
		...
		transType = (String)result.get(Tran.MANAGE_PREDETAIL_TRANSFERTYPE_RES);
		if(transType.equals("40")) {
			Intent intent=new Intent(this,RemittanceTransRecordDetailActivity.class);
			startActivity(intent);
		} else {
			Intent intent = new Intent(ManageTransRecordActivity1.this,
					TransRecordDetailActivity1.class);
			intent.putExtra(Tran.MANAGE_RECORDS_REEXCHANGESTATE_RES, reexchangestate);
			intent.putExtra(Tran.MANAGE_RECORDS_TRANSACTIONID_RES, transId);
			startActivity(intent);
		}
