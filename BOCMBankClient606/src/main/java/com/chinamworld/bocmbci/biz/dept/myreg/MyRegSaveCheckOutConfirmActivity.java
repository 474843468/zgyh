 ...
 * 存款管理 我的定期存款 "确认" & "成功页面"
 * 
 * @author 修改 luqp 2015年8月13日10:11:14
 */
public class MyRegSaveCheckOutConfirmActivity extends DeptBaseActivity {
	private static final String TAG = "MyRegSaveCheckOutConfirmActivity";
	...
	/** 周期 */
	private String cycleSelect = null;
	/** 开始日期 */
	...
	/** 约定支取日期 */
	private String preDrawDate = null;
	/** 续存金额 */
	private String continueSaveMoney = null;
	/** 转出账户号码 */
	private String strAccOutNumber = null;
	/** 转入账户号码 */
	private String strAccInNumber = null;
	/** 确定 */
	private String strConfirm = null;
	/** 当前系统时间一天后 */
	private String oneDateLater = null;
	/** 当前系统时间七天后 */
	private String sevenDateLater = null;
	/** 执行次数 */
	private int executeTimes;
	/** 支取方式 */
	private String strDrawMode = null;
	/** 到期日 */
	...
	private String callDepositBalance = null;
	/** 是否到期支取 */
	...
	private String appointedAmount = null;
	/** 是否是从我要存定期跳转过来的 */
	...
	private void aquireData() {
		...
		// 当前账户详情
		curDetail = DeptDataCenter.getInstance().getAccountContentMap();
		...
		// add 2015年11月9日19:20:44 luqp 约定支取日期
		...
		appointedAmount = (String) curDetail.get(Dept.DEPT_AVAILABLEBALANCE_RES);
		appointedAmount = StringUtil.parseStringCodePattern(currency, appointedAmount, 2).replaceAll(",", "");

		// //////////////////////////////////////////////////////////////////////////////////
		// // 手续费试算 start /////
		// /////////////////////////////////////////////////////////////////////////////////
		// 获取手续费试算数据 add by luqp 2015年11月23日10:56:18
		if (saveType.equals(EDUCATION_SAVE1) || saveType.equals(EDUCATION_SAVE2)) {
			// 获得教育储存手续费试算的map数据
			flagData = DeptDataCenter.getInstance().getEducationStockpileMap();
			...
			// 获得零存整取手续费试算的map数据
			flagData = DeptDataCenter.getInstance().getZeroDepositMap();
			if (flagData.containsKey(Dept.Save_needCommissionCharge)) {
				isShowBenchmark = true;
			}
			isChargeFlag = (String) flagData.get(Dept.Save_getChargeFlag);
...
//				ActivityTaskManager.getInstance().removeAllActivity();
				// by luqp 2016年12月7日 点击主界面返回到行方主界面
				goToMainActivity();
				finish();
			}
		});
		...
		case IMMEDIATELY_EXCUTE:// 不同存单类型 显示确定信息页面不同
			executeWayFlag = IMMEDIATELY_EXCUTE; // 立即执行
			showConfirmLayout();
			break;
	...
	private void showConfirmLayout() {
		...
		// if (RANDOM_SAVE.equals(saveType) || WHOE_SAVE.equals(saveType)) {
		// amount = (String) curDetail.get(Dept.AVAILABLE_BALANCE);
		// amount = StringUtil.parseStringPattern(amount, 2).replaceAll(",",
		// "");
		// }
	...
	private void showOneDateLayout() {
		...
		// if (!promiseWay.equals(ConstantGloble.CONVERTTYPE_N)) {// 非约定
		// // checkoutWayTv.setText(strCheckoutPart);
		// promiseCheckOutDay.setVisibility(View.GONE);
		// } else {
		// if (!isExpire)
		// promiseCheckOutDay.setVisibility(View.GONE);
	...
	private void showExtendEducationSuccessLayout() {
		...
		// ////////////初始化 基准费用 和优惠后费用/////////////////
		// add by luqp 2015年12月15日10:44:47  预约日期 预约周期 优惠后费用 试算接口取值
		if(Integer.parseInt(transMode) == PRE_DATE_EXCUTE || Integer.parseInt(transMode) == PRE_WEEK_EXCUTE){
			trialTrialChargeNoPrompted(isChargeFlag, needCommission, preCommission, remitSetMealFlag, isShowBenchmark);
		} else { // 立即执行提交交易页面取值
			trialTrialChargeNoPrompted(isChargeFlag, needCommission, preferential, remitSetMealFlag, isShowBenchmark);
		}
		...
		public void onClick(View v) {
			...
			if (!flag) {
				BaseDroidApp.getInstanse().showInfoMessageDialog(
						MyRegSaveCheckOutConfirmActivity.this.getResources().getString(R.string.execute_day));
				return;
			}
	...
	private OnClickListener preWeekNextBtnListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (!QueryDateUtils.compareStartDate(startDate, dateTime)) {
				BaseDroidApp.getInstanse().showInfoMessageDialog(
						MyRegSaveCheckOutConfirmActivity.this.getResources().getString(R.string.choose_start_day_info));
				return;
			}

			if (!QueryDateUtils.compareEndDate(endDate, dateTime)) {
				BaseDroidApp.getInstanse().showInfoMessageDialog(
						MyRegSaveCheckOutConfirmActivity.this.getResources().getString(R.string.choose_end_day_info));
				return;
			}
			boolean flag = QueryDateUtils.compareDate(startDate, endDate);
			if (!flag) {
				BaseDroidApp.getInstanse().showInfoMessageDialog(
						MyRegSaveCheckOutConfirmActivity.this.getResources().getString(R.string.acc_query_errordate));
				return;
			}
			btnConfirm.setText(strConfirm);
			// 执行次数
	...
	private OnClickListener successConBtnListener = new OnClickListener() {
		...
		public void onClick(View v) {
			// 返回行方账户列表页面
			if(DeptDataCenter.getInstance().isFromBoc){
				ActivityTaskManager.getInstance().removeAllActivity();
				ModelBoc.gotoAccountModuleActivity(MyRegSaveCheckOutConfirmActivity.this);
				finish();
				return;
			}
			ActivityTaskManager.getInstance().removeAllActivity();
			...
			if (isSaveRegEdu) {
				intent.setClass(MyRegSaveCheckOutConfirmActivity.this, SaveRegularActivity.class);
			} else {
				if (notifyFlag == 1) {
					intent.setClass(MyRegSaveCheckOutConfirmActivity.this, NotifyManageActivity.class);
				} else {
					intent.setClass(MyRegSaveCheckOutConfirmActivity.this, MyRegSaveActivity.class);
				}
			}
	...
	protected Dialog onCreateDialog(int id) {
		...
		case CHOOSE_DATE_PICKER_ID:
			DatePickerDialog chooseDateDialog = new DatePickerDialog(MyRegSaveCheckOutConfirmActivity.this,
					onChooseDateSetListener, startYear, startMonth - 1, startDay);
			return chooseDateDialog;
		case START_DATE_PICKER_ID:
			DatePickerDialog startDateDialog = new DatePickerDialog(MyRegSaveCheckOutConfirmActivity.this,
					onStartDateSetListener, startYear, startMonth - 1, startDay);
			return startDateDialog;
		case END_DATE_PICKER_ID:
			DatePickerDialog endDateDialog = new DatePickerDialog(MyRegSaveCheckOutConfirmActivity.this,
					onEndDateSetListener, endYear, endMonth - 1, endDay);
	...
	private void trialTrialCharge(String isChargeFlag, String needCommission, String preCommission, String remitSetMealFlag,
		...
		if (DeptBaseActivity.ONE.equals(isChargeFlag)) { // 查询手续费试算成功
			if (isShowBenchmark = true && !StringUtil.isNull(needCommission)) { // 基准费用不为空
				...
				// add by luqp 2015年12月11日15:14:00 预约周期修改为 单笔基准费用 单笔优惠后费用
				...
				// add by luqp 2015年12月11日15:14:00 预约周期修改为 单笔优惠后费用
				ll_discount_cost_prompted.setVisibility(View.VISIBLE);
				if (executeWayFlag == PRE_WEEK_EXCUTE) {
