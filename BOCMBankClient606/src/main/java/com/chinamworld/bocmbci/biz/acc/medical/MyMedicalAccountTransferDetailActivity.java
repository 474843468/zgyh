 ...
 * 
 */
public class MyMedicalAccountTransferDetailActivity extends AccBaseActivity
...
//			PopupWindowUtils.getInstance().showQueryPopupWindowFirst();
	...
	private void init() {
		// 取得账户列表信息
		bankList = AccDataCenter.getInstance().getMedicalAccountList();
		rl_query_transfer = view.findViewById(R.id.layout_queryviews);
...
//				R.layout.acc_querytransfer_condition, null);
		...
		gallery.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,
				...
				if (isfirst) {
					...
					// 请求余额
					requestAccBankAccountDetail(String.valueOf(currentBankList
	...
	public boolean httpRequestCallBackPre(Object resultObj) {
		...
		if (biiResponseBody.getStatus().equals(ConstantGloble.STATUS_FAIL)) {// 返回的是错误码
			if (Acc.ACC_MEDICALACCOUNTDETAIL_API.equals(biiResponseBody
					.getMethod())) {
	...
	public void commonHttpErrorCallBack(String requestMethod) {
		if (Acc.ACC_MEDICALACCOUNTDETAIL_API.equals(requestMethod)) {
			// 代表返回数据异常
	...
	public void requestAccBankAccountDetail(String accountId) {
		...
		biiRequestBody.setMethod(Acc.ACC_MEDICALACCOUNTDETAIL_API);
				...
				if (!StringUtil.isNullOrEmpty(queryCodeList)
					...
					if (mycurrency.equals(rmb)) {
						cashRemitadapter = new ArrayAdapter(
								MyMedicalAccountTransferDetailActivity.this,
...
//				PopupWindowUtils.getInstance().dissMissQueryPopupWindow();
		...
		public void onClick(View v) {
			...
			case R.id.tv_text2:
				...
				AccountTransferAdapter indetailadapter = new AccountTransferAdapter(
						MyMedicalAccountTransferDetailActivity.this, listIn,
			...
			case R.id.tv_text3:
				...
				AccountTransferAdapter outdetailadapter = new AccountTransferAdapter(
						MyMedicalAccountTransferDetailActivity.this, listOut,
						queryMap.get(Acc.QUERYTRANSFER_ACC_CURRENCY_REQ));
	...
	public void requestTransferList() {
		BiiRequestBody biiRequestBody = new BiiRequestBody();
		biiRequestBody.setMethod(Acc.MEDICAL_QUERYTRANSFERDETAIL_API);
...
//			PopupWindowUtils.getInstance().dissMissQueryPopupWindow();
			...
			detailadapter = new AccountTransferAdapter(
					MyMedicalAccountTransferDetailActivity.this, transferList,
					queryMap.get(Acc.QUERYTRANSFER_ACC_CURRENCY_REQ));
			lv_acc_query_result.setAdapter(detailadapter);
	...
	public void requestPsnInquiryRangeQuery(){
		...
		HttpManager.requestBii(biiRequestBody, this, "psnInquiryRangeQueryCallBack");
	}
	/**
	 * 查询最大跨度和最长时间范围返回
	 */
	public void psnInquiryRangeQueryCallBack(Object resultObj){
		...
		BaseHttpEngine.dissMissProgressDialog();
		maxYears=Integer.valueOf((String)queryMaxTime.get(Acc.ACC_QUERY_MAXYEARS));
		maxMonths=Integer.valueOf((String)queryMaxTime.get(Acc.ACC_QUERY_MAXMONTHS));
		queryDetail();
	}
	/** 右侧按钮点击事件 */
...
//				return;
//			}
			if(QueryDateUtils.compareDateMaxYears(startdate, dateTime, maxYears)){
				//起始日期不能早于系统当前日期maxYears年前
			}else{
				BaseDroidApp.getInstanse().showInfoMessageDialog("起始日期最早为系统当前日期"+AccDataCenter.getInstance().messageInfo
...
//				return;
//			}
			if(QueryDateUtils.compareDateMaxMonths(startdate, enddate, maxMonths)){
				//起始日期与结束日期最大间隔为maxMonths个自然月
			}else{
				BaseDroidApp.getInstanse().showInfoMessageDialog("查询范围最大为"+AccDataCenter.getInstance().messageInfo
