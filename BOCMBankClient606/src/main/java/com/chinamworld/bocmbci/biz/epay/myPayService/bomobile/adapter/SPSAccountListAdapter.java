...
public class SPSAccountListAdapter extends BaseAdapter {
	...
	@Override
	public int getCount() {
		return bomTransContext.getList(PubConstants.CONTEXT_FIELD_UN_DREDGED_ACCOUNT_LIST).size();
	}

	@Override
	public Object getItem(int position) {
		return bomTransContext.getList(PubConstants.CONTEXT_FIELD_UN_DREDGED_ACCOUNT_LIST).get(position);
	}
...
//			holder.img_gou = (ImageView) convertView.findViewById(R.id.img_gou);
		...
		Map<Object, Object> tempMap = EpayUtil.getMap(getItem(position));
		...
		if (!(-1 == selectedPosition)) {
			...
			if (selectedPosition != position) {
				...
				Map<Object, Object> selectedPositionMap = EpayUtil.getMap(getItem(selectedPosition));
