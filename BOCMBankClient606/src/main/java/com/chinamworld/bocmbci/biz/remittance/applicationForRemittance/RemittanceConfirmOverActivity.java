...
import com.chinamworld.bocmbci.biz.remittance.applicationForRemittance.RemittanceSuccessActivity;
...
public class RemittanceConfirmOverActivity extends RemittanceBaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		addView(R.layout.remittance_confirm_over);
...
//				get(RemittanceDataCenter.payeeAreaCode.indexOf(map.get(Remittance.GATHERINGAREA))));
		...
		if (map.get(Remittance.GATHERINGAREA).equals("JP")) {
			...
			//((TextView) findViewById(R.id.tv_payeeBankAdd)).setText((String) userInput.get(Remittance.PAYEEBANKADD)); 收款银行地址 无
		} else {
...
//		}
		...
		//((TextView) findViewById(R.id.tv_gatheringArea)).setText((String) map.get(Remittance.GATHERINGAREA));
		((TextView) findViewById(R.id.tv_gatheringArea)).setText(RemittanceDataCenter.getInstance().getbankSearsOver());

		String currency = LocalData.Currency.get(map.get(Remittance.REMITCURRENCYCODE));
...
//		((TextView) findViewById(R.id.tv_toPayeeMessage)).setText(StringUtil.valueOf1((String) userInput.get(Remittance.REMITFURINFO2PAYEE)));
	...
	public void requestPsnTransInternationalTransferSubmitCallBack(Object resultObj) {
		...
		startActivityForResult(new Intent(this, RemittanceSuccessOverActivity.class), QUIT_CODE);
	}
