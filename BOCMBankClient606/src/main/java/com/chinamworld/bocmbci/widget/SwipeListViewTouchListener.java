...
public class SwipeListViewTouchListener implements View.OnTouchListener {
	...
	private boolean lastPositionClickable;// add by wjp
    private boolean allPositionClickable;
	/**
	 * Constructor
	 * 
	 * @param swipeListView
...
//							}
							//add by wjp动画结束之后 自动返回来
							int moveTo = (int) (leftOffset);
							animate(view).translationX(moveTo).setDuration(animationTime);
						}
					}
				});
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		if(((SwipeListView)view).isDragging()){
			return false;
		}
		if (viewWidth < 2) {
		...
		case MotionEvent.ACTION_UP: {
			...
			if (!lastPositionClickable) {// add by wjp
				if (downPosition == swipeListView.getCount() - 1) {
					break;
				}
			}
			...
			//modify by wjp 2013-6-19 10:11:40
			if(velocityY < velocityX){
				if(!allPositionClickable){
					break;
				}
			}
			...
			if (minFlingVelocity <= velocityX && velocityX <= maxFlingVelocity
					&& velocityY < velocityX) {
				swapRight = velocityTracker.getXVelocity() > 100;
	...
	public void setLastPositionClickable(boolean clickable){
		lastPositionClickable = clickable;
	}

	public boolean isAllPositionClickable() {
		return allPositionClickable;
	}

	public void setAllPositionClickable(boolean allPositionClickable) {
		this.allPositionClickable = allPositionClickable;
	}
}
