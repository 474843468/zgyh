...
public class APPDownLoadService extends Service {
	...
	private static final String DOWNLOAD_PATH = "http://mentry.bocichina.com:8080/download/adkh.apk";
	private static final String DOWNLOADRECEIVER = "_com.chinamworold.downloadcancle.receiver";
	...
	private void downLoadClient(String url) {
		...
		String sdcardpath = android.os.Environment
				.getExternalStorageDirectory() + "/" + DOWNLOADFILENAME;
		final String fileName = sdcardpath + File.separator + "adkh" + ".apk";
		File file = new File(sdcardpath);
		if (!file.exists()) {
			try {
				file.mkdirs();
				if (!file.mkdirs())
					Toast.makeText(getApplicationContext(), "抱歉未检测到sd卡，下载失败",
					...
					public void onFailure(Throwable t, int errorNo,
						...
						if (errorNo == 416) {// maybe you have download complete
							...
							Toast.makeText(getApplicationContext(),
									"中银证券开户下载失败，请查看网络状态", Toast.LENGTH_SHORT)
									.show();
							stopSelf();
						}
					}
