...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	public void uncaughtException(Thread thread, Throwable ex) {
		if (!handleException(ex) && mDefaultHandler != null) {
			...
			// LogGloble.d(TAG, "用户未处理，系统默认的异常处理器未处理");
			// 用户未处理，系统默认的异常处理器未处理，最终弹出提示框
			// final BaseActivity baseActivity = BaseDroidApp.getInstanse()
			// .getCurrentAct();
			...
			// ConstantGloble.SYSTEM_0000B, R.string.system_crashed,
			// onclickListener);
			// } catch (Exception e) {
			...
			// // 休眠3秒后终止当前出现异常的activity   是否 可通过onclickListener关闭弹出框
			// try {
			// Thread.sleep(3000);
			// } catch (InterruptedException e) {
			// LogGloble.w(TAG, "InterruptedException", e);
			// }
			//
